            ...
            var next = function () {
                ...
                if (_currentOperations.length > 0) {
                    ...
                    perform(pkg, n, op, function (p) {
                        ...
                        switch (p.operation) {
                            case "zoom":
                                _sync && _slate.meteor && _slate.meteor.send({ type: 'onZoom', data: { id: p.id, zoomLevel: p.zoomLevel} });
                                break;
                            case "position":
                                _sync && _slate.meteor && _slate.meteor.send({ type: "onNodePositioned", data: { id: p.id, location: p.location, easing: p.easing} });
                                break;
                        }
                        next();
                    });
