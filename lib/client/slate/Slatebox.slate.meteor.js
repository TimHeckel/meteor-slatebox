(function ($s, $slate, $) {
    $slate.fn._meteor = function () {
        ...
        function process(pkg) {
            if ($s.localRecipients.length > 1) {
                ...
                for (var s in $s.localRecipients) {
                    _time += 100;
                    (function (rec, t) {
                        setTimeout(function () { rec["_"]["meteor"]["invoke"](pkg); }, t);
        ...
        function wire() {
            ...
            var _watch = Collaboration.find({ slateId: Session.get(pc.sessionName) }).observe({
                added: function(doc) {
                    if (doc.userId !== getUserId()) process(doc);
                }
            });
        ...
        function getUserId() {
            if (Meteor.userId) {
                ...
                return pc.userIdOverride;
            }
        }

        function addMessage(pkg, msg) {
            CollaborationMessages.insert({userId: getUserId(), msg: msg});
        };
        ...
        function send(pkg) {
            if (pc.allow) {
                ...
                if ($s.isFunction(_self._.options.onSlateChanged)) {
                    _self._.options.onSlateChanged.apply(this);
                }

                pkg.userId = getUserId();
                pkg.slateId = _self._.options.id;

                if (pc.localizedOnly === true) {
                    process(pkg);
                } else {
                    Collaboration.insert(pkg);
                }
            }
        };
